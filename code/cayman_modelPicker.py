...
def generateDataset(datafile):
    ...
    for question in questions:
        ...
        for name, model in models.vsm_models:
            ...
            guess, dist = model(glove, question, return_vec=True);
            
			# Model failed at guessing question
            #if(guess == -1 or guess == None): continue;
            if (guess == None): continue;
            
            # Choose model that has the smallest distance between its returned "answer" vector and the real answer
...
if __name__ == "__main__":
    ...
    inform("Finished importing! Training NGrams and Loading Glove...");
    unigrams, bigrams, cgrams = getGrams("../data/Holmes_Training_Data/norvig.txt")
